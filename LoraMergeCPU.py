...
output_dir      = "/workspace/Qwen3-30B-A3B-Edgebot-Merged"

print("Base model (local CPU):", base_model_path)
print("LoRA dir:", lora_dir)
print("Output dir:", output_dir)

# 1) Load base model on CPU only
print("Loading base model on CPU...")
base_model = AutoModelForCausalLM.from_pretrained(
    base_model_path,
    torch_dtype=torch.float32,      # safe for CPU
    device_map=None,                # no auto, no GPU
...
# 3) Wrap base model with LoRA structure
...
# 5) Merge and unload
print("Merging LoRA into base weights (CPU)...")
